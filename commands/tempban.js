...
module.exports = {
    run: async (message, args) => {
        ...
        if (!member) return message.channel.send('**Veuillez mentionner le membre à bannir**:x:')
        ...
        message.channel.send(`${member.user.tag} **a été banni pendant** ${humanizeDuration(duration, {language: 'fr'})} :white_check_mark:

        `)
        setTimeout(() => {
            message.guild.members.unban(member)
            message.channel.send(`${member.user.tag} **a été débanni** :white_check_mark:
